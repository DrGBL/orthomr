...
N_INT=${#INNER_INTERVALS[@]}
N_PROP=${#PROP_INTS[@]}
N_INST=${#INST_INTS[@]}
N_SS=${#SS_INTS[@]}

TOTAL_JOBS=$((N_INT * N_PROP * N_INST * N_SS))
...
OUTPUT_NAME="sim_inner${new_int[$SLURM_ARRAY_TASK_ID]}_nrep${N_REPLICATES}_prop_pleio_${new_prop[$SLURM_ARRAY_TASK_ID]}_inst${new_inst[$SLURM_ARRAY_TASK_ID]}_ss${new_ss[$SLURM_ARRAY_TASK_ID]}.rds"

echo "=========================================="
echo "Job Configuration:"
echo "  Inner interval: ${new_int[$SLURM_ARRAY_TASK_ID]}"
...
Rscript /scratch/richards/guillaume.butler-laporte/non_linear_mr/simulations/rscript_simulations.R \
  --inner_interval "${new_int[$SLURM_ARRAY_TASK_ID]}" \
